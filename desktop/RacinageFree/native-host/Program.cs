  ...
  internal sealed partial class LocalServer {
    ...
    private void Handle(HttpListenerContext context) {
      try {
        ...
        if (path == "/assets/floaty.css") { WriteFile(context, "assets\\floaty.css", "text/css; charset=utf-8"); return; }
        if (path == "/assets/floaty.js") { WriteFile(context, "assets\\floaty.js", "application/javascript; charset=utf-8"); return; }
        ...
        if (path == "/manage" || path == "/manage/plugins" || path == "/manage/family" || path == "/manage/progress" || path == "/manage/settings" || path == "/manage/ai") { Manage(context, path); return; }
        if (path == "/local-ai-api") { LocalAiApi(context); return; }
        if (path == "/local-floaty-api") { LocalFloatyApi(context); return; }
    ...
    private void Manage(HttpListenerContext context, string path) {
      ...
      if (context.Request.HttpMethod == "POST") {
        ...
        else if (action == "save_progress_settings") { store.SaveProgressVisualMode(form.ContainsKey("visual_mode")?form["visual_mode"]:"subtle");message="Progress display settings saved."; }
        if (asynchronous) {
          string panel = action == "save_progress_settings" ? ProgressPanel() : PluginsPanel();
          WriteJson(context,json.Serialize(new Dictionary<string,object>{{"ok",true},{"message",message},{"html",panel}}));
          return;
        }
      }
    ...
    private string ManagePage(string path, string message) {
      string active = path.EndsWith("/plugins", StringComparison.OrdinalIgnoreCase) ? "plugins" : (path.EndsWith("/progress", StringComparison.OrdinalIgnoreCase) ? "progress" : (path.EndsWith("/settings", StringComparison.OrdinalIgnoreCase) ? "settings" : (path.EndsWith("/family", StringComparison.OrdinalIgnoreCase) ? "family" : (path.EndsWith("/ai", StringComparison.OrdinalIgnoreCase) ? "ai" : "account"))));
      string tabs = "<nav class='manage-tabs' aria-label='Manage sections'>" + ManageTab("/manage", "Account", active == "account") + ManageTab("/manage/family", "Family", active == "family") + ManageTab("/manage/progress", "Progress", active == "progress") + ManageTab("/manage/plugins", "Plugins", active == "plugins") + ManageTab("/manage/ai", "AI Features", active == "ai") + ManageTab("/manage/settings", "Settings", active == "settings") + "</nav>";
      string content;
      if (active == "plugins") content = PluginsPanel();
      else if (active == "ai") content = PortableAiSetup();
      else if (active == "progress") content = ProgressPanel();
    ...
    private string ProgressPanel() {
      Dictionary<string,object> summary=store.ProgressSummary();
      List<Dictionary<string,string> > weeks=(List<Dictionary<string,string> >)summary["weeks"];
      HashSet<string> earned=new HashSet<string>(((List<string>)summary["badges"]),StringComparer.OrdinalIgnoreCase);
      StringBuilder strip=new StringBuilder();foreach(Dictionary<string,string> week in weeks)strip.Append("<span class='progress-week "+(week["qualified"]=="1"?"is-qualified":"")+"' title='Week starting "+A(week["week_start"])+"'><b>"+H(week["label"])+"</b><small>"+(week["qualified"]=="1"?"Active":"Quiet")+"</small></span>");
      Dictionary<string,string> badgeNames=new Dictionary<string,string>{{"first_root","First Root"},{"tree_keeper","Tree Keeper"},{"century_canopy","Century Canopy"},{"memory_keeper","Memory Keeper"},{"plugin_pathfinder","Plugin Pathfinder"},{"weekly_gardener_4","Weekly Gardener IV"},{"weekly_gardener_12","Weekly Gardener XII"},{"weekly_gardener_26","Weekly Gardener XXVI"},{"weekly_gardener_52","Weekly Gardener LII"},{"weekly_gardener_104","Weekly Gardener CIV"}};
      StringBuilder badges=new StringBuilder();int tier=0;foreach(KeyValuePair<string,string> badge in badgeNames){tier++;bool has=earned.Contains(badge.Key);badges.Append("<article class='progress-badge tier-"+Math.Min(6,1+(tier-1)/2).ToString(CultureInfo.InvariantCulture)+" "+(has?"is-earned":"is-locked")+"'><span aria-hidden='true'>"+(has?"&#10003;":"&#9675;")+"</span><div><strong>"+H(badge.Value)+"</strong><small>"+(has?"Earned on this device":"Locked")+"</small></div></article>");}
      string mode=Convert.ToString(summary["visual_mode"],CultureInfo.InvariantCulture);Func<string,string,string> option=(value,label)=>"<option value='"+A(value)+"'"+(mode==value?" selected":"")+">"+H(label)+"</option>";
      return "<section class='manage-grid progress-summary'><article class='manage-card progress-level'><p class='kicker'>Local level</p><h2>"+H(Convert.ToString(summary["level"],CultureInfo.InvariantCulture))+"</h2><strong>"+H(Convert.ToString(summary["score"],CultureInfo.InvariantCulture))+" score</strong><p>Private analytics stored only on this Windows device.</p></article><article class='manage-card'><h2>Weekly consistency</h2><dl class='facts'><div><dt>Current streak</dt><dd>"+H(Convert.ToString(summary["current_streak"],CultureInfo.InvariantCulture))+" weeks</dd></div><div><dt>Longest streak</dt><dd>"+H(Convert.ToString(summary["longest_streak"],CultureInfo.InvariantCulture))+" weeks</dd></div></dl></article></section>"
        +"<section class='manage-card progress-strip-card'><div class='manage-card-head'><div><h2>Last 12 weeks</h2><p>One retained, trusted local action qualifies a week.</p></div></div><div class='progress-week-strip'>"+strip.ToString()+"</div></section>"
        +"<section class='manage-card'><div class='manage-card-head'><div><h2>Badge cabinet</h2><p>Badges recognise retained people, meaningful notes, reviewed plugin installs, and consistent weeks.</p></div><span class='status-pill'>"+earned.Count.ToString(CultureInfo.InvariantCulture)+" earned</span></div><div class='progress-badges'>"+badges.ToString()+"</div></section>"
        +"<section class='manage-grid'><article class='manage-card'><h2>Interface display</h2><p>Background hides decoration. Subtle adds a small level marker. Expressive adds static colour accents without motion.</p><form class='progress-action' method='post' action='/manage/progress'>"+CsrfInput()+"<input type='hidden' name='action' value='save_progress_settings'><label>Display mode<select name='visual_mode'>"+option("background","Background only")+option("subtle","Subtle")+option("expressive","Expressive")+"</select></label><button class='button' type='submit'>Save display mode</button></form></article><article class='manage-card progress-privacy'><h2>Offline privacy boundary</h2><p>There is no public alias, leaderboard, Canopy Points, reward catalogue, plan boost, hosted AI credit, or Progress synchronisation in Racinage Free.</p><p>The SQLite database remains under <code>%LOCALAPPDATA%\\Racinage Free</code> and is not exposed to hosted services or companion jobs.</p></article></section>";
    }
    ...
    private void PortablePlugin(HttpListenerContext context, string slug) {
      ...
      System.Text.RegularExpressions.Match head = System.Text.RegularExpressions.Regex.Match(source, @"<head(?:\s[^>]*)?>", System.Text.RegularExpressions.RegexOptions.IgnoreCase | System.Text.RegularExpressions.RegexOptions.CultureInvariant);
      if (!head.Success) { WriteHtml(context, Page("Plugin unavailable", "<section class='panel'><h1>Plugin unavailable</h1><p>This local plugin does not meet the secure document requirements.</p><a class='button' href='/manage/plugins'>Back to plugins</a></section>"), 422); return; }
      source = source.Insert(head.Index + head.Length, pluginCsp);
    ...
    private string Page(string title, string body) {
      ...
      string progressClass = store.ProgressBodyClass();
      return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>" +
        "<title>" + H(title) + " - Racinage Free</title><link rel='stylesheet' href='/assets/ai-assistant.css?v=" + assetVersion + "'><link rel='stylesheet' href='/assets/floaty.css?v=" + assetVersion + "'><style>" + Css() + CalendarCss() + ShareCss() + ProgressCss() + "</style></head><body class='" + A(progressClass) + "'>" +
        ...
        "<main>" + body + "</main><div id='racinage_floaty_shell' data-csrf='" + A(store.CsrfToken) + "'></div><script>" + PluginLifecycleJs() + CalendarJs() + Js() + connected.Script(store.CsrfToken) + "</script><script src='/assets/floaty.js?v=" + assetVersion + "'></script><script src='/assets/ai-assistant.js?v=" + assetVersion + "'></script></body></html>";
    }
...
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Tahoma,sans-serif;background:#f8fbfc;color:var(--text)}a{color:#007584;text-decoration:none}header{height:58px;display:flex;align-items:center;justify-content:space-between;padding:0 28px;border-bottom:1px solid var(--line);background:#fff;position:sticky;top:0;z-index:5}.brand{font-weight:800;color:var(--brand)}nav{display:flex;gap:16px;align-items:center}nav a{font-size:14px;font-weight:600;color:var(--muted)}main{max-width:1120px;margin:0 auto;padding:34px 24px 70px}.hero{min-height:360px;display:grid;align-items:center;border-bottom:1px solid var(--line)}.hero h1,.dashhead h1,.manage-head h1{font-size:48px;line-height:1.02;margin:6px 0 16px;color:var(--brand)}.hero p,.dashhead p,.manage-head p,.note{font-size:17px;line-height:1.6;max-width:760px;color:var(--muted)}.kicker{font-size:12px!important;text-transform:uppercase;letter-spacing:.14em;color:var(--accent)!important;font-weight:800;margin:0}.actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:20px}.actions form{display:block}.button{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:0 18px;border-radius:8px;border:1px solid var(--brand);background:var(--brand);color:#fff;font:700 14px/1 inherit;cursor:pointer}.button.ghost{background:transparent;color:var(--brand);border-color:#9ab2b8}.grid,.manage-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;margin-top:28px}.grid article,.panel,.manage-card{background:#fff;border:1px solid var(--line);border-radius:12px;padding:24px}.grid h2,.panel h2,.manage-card h2{margin:0 0 10px;color:var(--brand)}.grid p,.panel p,.manage-card p{line-height:1.55;color:var(--muted)}.narrow{max-width:460px;margin:30px auto}.wide{margin-top:18px}.layout{display:grid;grid-template-columns:1fr 1fr;gap:18px}.dashhead{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:22px}form{display:grid;gap:12px}label{display:grid;gap:6px;font-size:13px;font-weight:700;color:var(--brand)}input,textarea{width:100%;border:1px solid #cad8dd;border-radius:8px;padding:10px 12px;font:inherit;background:#fbfdfd;color:var(--text)}textarea{resize:vertical}.error{border:1px solid #efb5b5;background:#fff2f2;color:#9b2525;border-radius:8px;padding:10px 12px}.sharebar{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0 18px}.share{border:1px solid #cddbe0;background:#f4faff;border-radius:8px;padding:8px 11px;cursor:pointer;font-weight:700;color:var(--brand)}.people{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}.people article{border:1px solid var(--line);border-radius:10px;padding:14px;background:#fbfdfd}.people strong{display:block;color:var(--brand)}.people span{display:block;color:var(--accent);font-size:12px;font-weight:800;text-transform:uppercase;margin-top:3px}.people p{margin:8px 0 0;font-size:14px}.textbtn{border:0;background:transparent;color:#b93333;padding:8px 0 0;cursor:pointer;font-weight:700}.empty{margin:0}.modal{position:fixed;inset:0;background:rgba(5,21,25,.55);display:grid;place-items:center;padding:24px;z-index:20}.modal[hidden]{display:none}.modalbox{width:min(440px,100%);background:#fff;border-radius:12px;padding:24px;border:1px solid var(--line)}.modalbox h2{margin:0 0 10px;color:var(--brand)}.panelhead,.manage-card-head{display:flex;justify-content:space-between;gap:16px;align-items:center}.panelhead h2,.panelhead p,.manage-card-head h2,.manage-card-head p{margin:0}.manage-head{margin-bottom:22px}.manage-tabs{display:flex;gap:6px;overflow:auto;padding:5px;border:1px solid var(--line);border-radius:10px;background:#fff}.manage-tabs a{min-height:42px;display:inline-flex;align-items:center;padding:0 16px;border-radius:7px;font-weight:700;color:var(--muted)}.manage-tabs a.active{color:#fff;background:var(--brand)}.manage-content{margin-top:18px}.manage-grid{margin-top:0}.facts{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin:18px 0 0}.facts div{padding:12px;border:1px solid var(--line);border-radius:9px}.facts dt{font-size:12px;color:var(--muted)}.facts dd{margin:5px 0 0;color:var(--brand);font-weight:700}.plugin-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:12px;margin-top:18px}.plugin-card{display:flex;flex-direction:column;min-height:260px;padding:16px;border:1px solid var(--line);border-radius:10px;background:#fbfdfd}.plugin-card-top{display:flex;gap:11px;align-items:center}.plugin-card h3{margin:0;color:var(--brand)}.plugin-card>p{flex:1}.plugin-mark{width:44px;height:44px;display:grid;place-items:center;border-radius:9px;color:#fff;background:var(--brand);font-size:20px;font-weight:800}.plugin-meta{margin:4px 0 0!important;font-size:12px}.notice{padding:10px;border-left:3px solid var(--accent);background:#fff8ef;font-size:13px}.status-pill{padding:7px 10px;border-radius:999px;color:var(--brand);background:#e9f3ef;font-size:12px;font-weight:800}.plugin-frame{width:100%;min-height:620px;border:1px solid var(--line);border-radius:12px;background:#fff}.connected-status{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-bottom:16px}.connected-status h2,.connected-status p{margin:0}.connected-layout{display:grid;grid-template-columns:280px minmax(0,1fr);gap:16px}.connected-layout aside{display:grid;align-content:start;gap:6px;padding:10px}.connected-conversation{display:grid;gap:3px;padding:12px;border:1px solid transparent;border-radius:8px;color:var(--brand)}.connected-conversation span{font-size:12px;color:var(--muted)}.connected-conversation.active{border-color:var(--line);background:var(--pale)}.connected-thread{display:grid;gap:18px}.connected-thread>div{display:grid;gap:8px;max-height:520px;overflow:auto}.connected-message{padding:12px;border:1px solid var(--line);border-radius:9px;background:#fbfdfd}.connected-message>div{display:flex;justify-content:space-between;gap:12px}.connected-message time{font-size:12px;color:var(--muted)}.connected-message p{margin:8px 0 0;white-space:normal}.connected-attachments{margin:8px 0 0;padding-left:20px;font-size:12px;color:var(--muted)}.connected-composer{padding-top:16px;border-top:1px solid var(--line)}@media(max-width:760px){header{padding:0 16px}nav{gap:10px}.hero h1,.dashhead h1,.manage-head h1{font-size:36px}.layout,.connected-layout{grid-template-columns:1fr}.dashhead,.manage-card-head,.connected-status{display:block}.manage-card-head .button,.manage-card-head .status-pill,.connected-status .actions{margin-top:12px}}";
    ...
    private static string ProgressCss() {
      return @"
.progress-subtle{--progress-color:#2f8173}.progress-expressive{--progress-color:#c35900}.progress-level-sprout{--progress-color:#398c65}.progress-level-branch{--progress-color:#267f87}.progress-level-grove{--progress-color:#6d739e}.progress-level-canopy{--progress-color:#9b6a2c}.progress-level-legacy{--progress-color:#6f4a7f}.progress-subtle .brand:after,.progress-expressive .brand:after{content:'';display:inline-block;width:9px;height:9px;margin-left:7px;border:2px solid var(--progress-color);border-radius:50%}.progress-expressive #header{border-bottom-color:var(--progress-color)}.progress-expressive .manage-head{border-left:4px solid var(--progress-color);padding-left:16px}.progress-summary{grid-template-columns:minmax(230px,.72fr) minmax(300px,1.28fr)}.progress-level h2{font-size:34px;margin-bottom:4px}.progress-level>strong{color:var(--progress-color,var(--brand));font-size:18px}.progress-strip-card{margin-top:18px}.progress-week-strip{display:grid;grid-template-columns:repeat(12,minmax(58px,1fr));gap:7px;margin-top:18px}.progress-week{display:grid;gap:4px;min-height:58px;padding:9px 6px;border:1px solid var(--line);border-radius:8px;text-align:center;background:#fbfdfd}.progress-week b{font-size:11px;color:var(--brand)}.progress-week small{font-size:10px;color:var(--muted)}.progress-week.is-qualified{border-color:var(--progress-color,var(--brand));background:#eef8f4}.progress-week.is-qualified small{color:var(--brand);font-weight:700}.progress-badges{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:9px;margin-top:18px}.progress-badge{display:flex;align-items:center;gap:11px;min-height:66px;padding:11px;border:1px solid var(--line);border-radius:10px;background:#fbfdfd}.progress-badge>span{display:grid;place-items:center;width:38px;height:38px;border:2px solid #b5c4c7;border-radius:50%;color:#829092}.progress-badge strong,.progress-badge small{display:block}.progress-badge small{margin-top:3px;color:var(--muted)}.progress-badge.is-earned{border-color:var(--progress-color,var(--brand))}.progress-badge.is-earned>span{border-width:3px;border-color:var(--progress-color,var(--brand));color:var(--progress-color,var(--brand))}.progress-badge.tier-4>span,.progress-badge.tier-5>span,.progress-badge.tier-6>span{border-style:double}.progress-badge.is-locked{opacity:.68}.progress-privacy{border-left:3px solid #c98032}.progress-privacy code{word-break:break-word}
@media(max-width:760px){.progress-summary{grid-template-columns:1fr}.progress-week-strip{grid-template-columns:repeat(4,minmax(60px,1fr))}}
";
    }

    private string Js() {
      return "function showUpgrade(feature){var m=document.getElementById('upgradeModal');document.getElementById('upgradeFeature').textContent=feature;m.hidden=false;}function hideUpgrade(){document.getElementById('upgradeModal').hidden=true;}document.addEventListener('keydown',function(e){if(e.key==='Escape')hideUpgrade();});document.addEventListener('click',function(e){var p=e.target.closest&&e.target.closest('input[type=date],input[type=datetime-local],input[type=time],input[type=month],input[type=year]');if(!p||p.disabled||p.readOnly||typeof p.showPicker!=='function')return;try{p.showPicker();}catch(_){}});document.addEventListener('submit',async function(e){var f=e.target.closest&&e.target.closest('.plugin-action,.progress-action');if(!f)return;e.preventDefault();if(f.dataset.busy==='1')return;f.dataset.busy='1';try{var r=await fetch(f.action,{method:'POST',credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest'},body:new URLSearchParams(new FormData(f))}),j=await r.json();if(!r.ok||!j.ok)throw new Error(j.message||'Action failed.');var c=document.querySelector('.manage-content');if(c)c.innerHTML=j.html;var x=document.querySelector('.error');if(x){x.textContent=j.message;x.className='feedback';}}catch(x){var c=document.querySelector('.manage-content');if(c)c.insertAdjacentHTML('afterbegin','<p class=\"error\"></p>');var p=document.querySelector('.manage-content .error');if(p)p.textContent=x.message;}finally{delete f.dataset.busy;}});window.addEventListener('message',async function(e){var f=document.querySelector('.plugin-frame[data-bridge-token]'),m=e.data,legacy=m&&m.financeBridge;if(!f||e.source!==f.contentWindow||!m||(!m.pluginBridge&&!legacy)||m.slug!==f.dataset.pluginSlug||m.bridgeToken!==f.dataset.bridgeToken)return;var reply={pluginBridgeResponse:true,financeBridgeResponse:true,bridgeToken:f.dataset.bridgeToken,requestId:m.requestId,ok:false};try{var r=await fetch('/local-plugin-api/'+encodeURIComponent(m.slug),{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-Racinage-CSRF':'" + A(store.CsrfToken) + "'},body:JSON.stringify({action:m.action,payload:m.payload||{}})}),j=await r.json();reply.ok=!!j.ok;reply.result=j.result;reply.message=j.message;}catch(_){reply.message='The local plugin service could not complete the request.';}f.contentWindow.postMessage(reply,'*');});";
    }
  ...
  internal sealed partial class LocalStore {
    ...
    internal void Initialize() {
      using (SqliteDb db = Open()) {
        ...
        db.Exec("CREATE TABLE IF NOT EXISTS local_progress_profile (id INTEGER PRIMARY KEY CHECK(id=1),visual_mode TEXT NOT NULL DEFAULT 'subtle' CHECK(visual_mode IN('background','subtle','expressive')),all_time_score INTEGER NOT NULL DEFAULT 0,current_streak INTEGER NOT NULL DEFAULT 0,longest_streak INTEGER NOT NULL DEFAULT 0,last_qualified_week TEXT NOT NULL DEFAULT '',level_code TEXT NOT NULL DEFAULT 'seed',updated_at TEXT NOT NULL)");
        db.Exec("INSERT OR IGNORE INTO local_progress_profile(id,updated_at)VALUES(1,'" + Now() + "')");
        db.Exec("CREATE TABLE IF NOT EXISTS local_progress_events (id INTEGER PRIMARY KEY AUTOINCREMENT,event_key TEXT NOT NULL,source_type TEXT NOT NULL,source_id TEXT NOT NULL,idempotency_key TEXT NOT NULL UNIQUE,score_value INTEGER NOT NULL CHECK(score_value>=0),occurred_at TEXT NOT NULL,week_start TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'awarded' CHECK(status IN('awarded','suppressed')))");
        db.Exec("CREATE INDEX IF NOT EXISTS idx_local_progress_events_week ON local_progress_events(week_start,event_key,status)");
        db.Exec("CREATE TABLE IF NOT EXISTS local_progress_weekly_rollups (week_start TEXT PRIMARY KEY,score INTEGER NOT NULL DEFAULT 0,qualified INTEGER NOT NULL DEFAULT 0 CHECK(qualified IN(0,1)),updated_at TEXT NOT NULL)");
        db.Exec("CREATE TABLE IF NOT EXISTS local_progress_badges (badge_key TEXT PRIMARY KEY,awarded_at TEXT NOT NULL,source_event_id INTEGER NULL,FOREIGN KEY(source_event_id) REFERENCES local_progress_events(id) ON DELETE SET NULL)");
        ...
        db.Exec("CREATE TABLE IF NOT EXISTS local_floaty_windows (long_id TEXT PRIMARY KEY,title TEXT NOT NULL,scope TEXT NOT NULL DEFAULT 'all_pages',route_key TEXT NOT NULL DEFAULT '',x_normalized REAL NOT NULL DEFAULT 1,y_normalized REAL NOT NULL DEFAULT 1,width_px INTEGER NOT NULL DEFAULT 360,height_px INTEGER NOT NULL DEFAULT 420,minimized INTEGER NOT NULL DEFAULT 0,stack_order INTEGER NOT NULL DEFAULT 0,revision INTEGER NOT NULL DEFAULT 1,status TEXT NOT NULL DEFAULT 'active',updated_at TEXT NOT NULL)");
        db.Exec("CREATE TABLE IF NOT EXISTS local_floaty_items (window_long_id TEXT NOT NULL,provider TEXT NOT NULL,record_type TEXT NOT NULL,record_long_id TEXT NOT NULL,view_json TEXT NOT NULL,revision INTEGER NOT NULL DEFAULT 1,status TEXT NOT NULL DEFAULT 'active',updated_at TEXT NOT NULL,PRIMARY KEY(window_long_id,provider,record_type,record_long_id))");
    ...
    internal void SaveCalendarItem(Dictionary<string,string> form) {
      ...
      if(created)RecordProgressEvent("calendar_item_created","calendar",longId,20);
      ProtectDatabaseFile();
    }
    ...
    private void EnsureBuiltinFinanceManager() {
      ...
      using(SqliteDb db=Open()){
        ...
        if(row==null)db.Execute("INSERT INTO plugin_installs(slug,name,version,checksum_sha256,entrypoint,status,installed_at,updated_at,bridge_operations)VALUES('finance-manager','Finance Manager','1.4.0',?,'index.html','enabled',?,?,'bootstrap,save,batch_save,delete,settings,post_quick_expense,attachment_upload,attachment_get,attachment_delete')",checksum,now,now);
        else db.Execute("UPDATE plugin_installs SET name='Finance Manager',version='1.4.0',checksum_sha256=?,entrypoint='index.html',bridge_operations='bootstrap,save,batch_save,delete,settings,post_quick_expense,attachment_upload,attachment_get,attachment_delete',updated_at=? WHERE slug='finance-manager'",checksum,now);
      }
    }
    ...
    internal object LocalPluginAction(string slug,string action,Dictionary<string,object> payload) {
      ...
      if(slug=="finance-manager"){
        ...
        if(action=="post_quick_expense")return FinancePostQuickExpense(slug,payload);
    ...
    private static string[] KnownPluginOperations(string slug){
      if(slug=="finance-manager")return new[]{"bootstrap","save","batch_save","delete","settings","post_quick_expense","attachment_upload","attachment_get","attachment_delete"};
    ...
    private static readonly HashSet<string> FinanceRecordTypes=new HashSet<string>(StringComparer.Ordinal){"workspaces","accounts","transactions","categories","tags","recurring_rules","budgets","goals","debts","debt_payments","investments","scenarios","circles","circle_members","circle_entries","quick_expenses","quick_expense_entries","quick_expense_postings"};
    ...
    private void ValidateFinanceData(SqliteDb db,string slug,string type,string workspace,Dictionary<string,object> values,string longId) {
      if(new[]{"workspaces","accounts","categories","tags","recurring_rules","budgets","goals","debts","investments","scenarios","circles","circle_members","quick_expenses"}.Contains(type)){string name=GetString(values,"name").Trim();if(name==""||name.Length>190)throw new InvalidDataException("A finance name is required.");}
      ...
      if(type=="quick_expenses"){ValidateCurrency(db,values);ValidateSnapshot(values,"starting_cents","starting_usd_cents");if(!new[]{"active","settled","archived"}.Contains(GetString(values,"quick_status")))throw new InvalidDataException("Choose a valid Quick Expense status.");}
      if(type=="quick_expense_entries"){if(GetLong(values,"amount_cents")<=0)throw new InvalidDataException("The Quick Expense amount must be positive.");ValidateCurrency(db,values);ValidateSnapshot(values,"amount_cents","amount_usd_cents");RequireReference(db,slug,workspace,"quick_expenses",GetString(values,"quick_expense"));string label=GetString(values,"label").Trim();if(label==""||label.Length>190)throw new InvalidDataException("An expense label is required.");DateTime occurred;if(!DateTime.TryParse(GetString(values,"occurred_at"),CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind,out occurred))throw new InvalidDataException("Choose a valid expense date and time.");}
      if(type=="quick_expense_postings"){RequireReference(db,slug,workspace,"quick_expenses",GetString(values,"quick_expense"));RequireReference(db,slug,workspace,"transactions",GetString(values,"transaction"));if(!new[]{"individual","summary","correction"}.Contains(GetString(values,"posting_mode")))throw new InvalidDataException("Choose a valid posting mode.");}
    }
    ...
    private object FinancePostQuickExpense(string slug,Dictionary<string,object> payload) {
      string quickId=SafeLongId(GetString(payload,"quick_expense")),accountId=SafeLongId(GetString(payload,"account")),categoryId=SafeLongId(GetString(payload,"category")),postingDate=GetString(payload,"posting_date"),idempotency=GetString(payload,"idempotency_key").Trim();int expected=GetInt(payload,"source_version");
      if(quickId==""||accountId==""||categoryId==""||!ValidDate(postingDate)||idempotency.Length<16||idempotency.Length>80)throw new InvalidDataException("The Quick Expense posting request is invalid.");
      foreach(char c in idempotency)if(!Char.IsLetterOrDigit(c)&&c!='-'&&c!='_')throw new InvalidDataException("The posting request identifier is invalid.");
      using(SqliteDb db=Open()){db.Exec("BEGIN IMMEDIATE");try{
        Dictionary<string,string> prior=db.QueryOne("SELECT long_id,data_json FROM local_plugin_records WHERE slug=? AND record_type='quick_expense_postings' AND status='active' AND json_extract(data_json,'$.idempotency_key')=? LIMIT 1",slug,idempotency);
        if(prior!=null){Dictionary<string,object> priorData=json.DeserializeObject(prior["data_json"]) as Dictionary<string,object>??new Dictionary<string,object>();if(GetString(priorData,"quick_expense")!=quickId||GetString(priorData,"account")!=accountId||GetString(priorData,"category")!=categoryId||GetString(priorData,"posting_date")!=postingDate)throw new InvalidOperationException("This posting key was already used for different values.");db.Exec("COMMIT");return new Dictionary<string,object>{{"posting_long_id",prior["long_id"]},{"transaction_long_id",GetString(priorData,"transaction")},{"duplicate",true}};}
        Dictionary<string,string> quickRow=db.QueryOne("SELECT workspace_long_id,data_json,version FROM local_plugin_records WHERE slug=? AND record_type='quick_expenses' AND long_id=? AND status='active' LIMIT 1",slug,quickId);if(quickRow==null)throw new InvalidOperationException("The Quick Expense is unavailable.");if(ToInt(quickRow["version"])!=expected)throw new InvalidOperationException("This Quick Expense changed. Reload it before posting.");
        string workspace=quickRow["workspace_long_id"];Dictionary<string,object> quick=json.DeserializeObject(quickRow["data_json"]) as Dictionary<string,object>??new Dictionary<string,object>();RequireReference(db,slug,workspace,"accounts",accountId);RequireReference(db,slug,workspace,"categories",categoryId);
        Dictionary<string,string> accountRow=db.QueryOne("SELECT data_json FROM local_plugin_records WHERE slug=? AND record_type='accounts' AND long_id=? AND workspace_long_id=? AND status='active' LIMIT 1",slug,accountId,workspace);Dictionary<string,object> account=json.DeserializeObject(accountRow["data_json"]) as Dictionary<string,object>??new Dictionary<string,object>();string currencyCode=GetString(account,"native_currency_code").ToUpperInvariant();Dictionary<string,string> rateRow=db.QueryOne("SELECT rate FROM local_currency_rates WHERE code=? AND rate>0 LIMIT 1",currencyCode);if(rateRow==null)throw new InvalidOperationException("The account currency is unavailable.");double rate=Double.Parse(rateRow["rate"],CultureInfo.InvariantCulture);
        long spentUsd=0;int entryCount=0;foreach(Dictionary<string,string> entryRow in db.Query("SELECT data_json FROM local_plugin_records WHERE slug=? AND record_type='quick_expense_entries' AND workspace_long_id=? AND status='active' AND json_extract(data_json,'$.quick_expense')=? ORDER BY long_id",slug,workspace,quickId)){Dictionary<string,object> entry=json.DeserializeObject(entryRow["data_json"]) as Dictionary<string,object>??new Dictionary<string,object>();spentUsd+=GetLong(entry,"amount_usd_cents");entryCount++;}if(entryCount<1||spentUsd<=0)throw new InvalidOperationException("Record an expense before posting.");
        long native=(long)Math.Round(spentUsd*rate,MidpointRounding.AwayFromZero);string now=Now();Dictionary<string,object> txData=new Dictionary<string,object>{{"transaction_type","expense"},{"account",accountId},{"destination_account",""},{"category",categoryId},{"value",(native/100.0).ToString("0.00",CultureInfo.InvariantCulture)},{"amount_cents",native},{"amount_usd_cents",spentUsd},{"fx_rate",rate},{"native_currency_code",currencyCode},{"transaction_date",postingDate},{"payee",GetString(quick,"name")},{"note","Posted from Quick Expense " + GetString(quick,"name")},{"tags","Quick Expense"},{"state","cleared"},{"source","quick_expense"},{"source_quick_expense",quickId},{"splits",new object[0]}};
        Dictionary<string,object> tx=FinanceSaveRecord(db,slug,new Dictionary<string,object>{{"record_type","transactions"},{"workspace_long_id",workspace},{"data",txData}});Dictionary<string,object> postingData=new Dictionary<string,object>{{"quick_expense",quickId},{"transaction",GetString(tx,"long_id")},{"account",accountId},{"category",categoryId},{"posting_date",postingDate},{"posting_mode","summary"},{"source_version",expected},{"total_usd_cents",spentUsd},{"entry_count",entryCount},{"idempotency_key",idempotency},{"posted_at",now}};Dictionary<string,object> posting=FinanceSaveRecord(db,slug,new Dictionary<string,object>{{"record_type","quick_expense_postings"},{"workspace_long_id",workspace},{"data",postingData}});
        db.Exec("COMMIT");ProtectDatabaseFile();return new Dictionary<string,object>{{"posting_long_id",GetString(posting,"long_id")},{"transaction_long_id",GetString(tx,"long_id")},{"duplicate",false}};
      }catch{db.Exec("ROLLBACK");throw;}}
    }
    ...
    private static DateTime ProgressWeekStart(DateTime localDate){int offset=((int)localDate.DayOfWeek+6)%7;return localDate.Date.AddDays(-offset);}
    private static string ProgressLevel(int score){if(score>=6000)return "legacy";if(score>=3000)return "canopy";if(score>=1500)return "grove";if(score>=750)return "branch";if(score>=250)return "sprout";return "seed";}
    private static void AwardLocalBadge(SqliteDb db,string key,long sourceEventId){db.Execute("INSERT OR IGNORE INTO local_progress_badges(badge_key,awarded_at,source_event_id)VALUES(?,?,?)",key,Now(),sourceEventId);}

    private void RecordProgressEvent(string eventKey,string sourceType,string sourceId,int score){
      if(String.IsNullOrWhiteSpace(eventKey)||String.IsNullOrWhiteSpace(sourceType)||String.IsNullOrWhiteSpace(sourceId)||score<0)return;
      eventKey=eventKey.Trim().ToLowerInvariant();sourceType=sourceType.Trim().ToLowerInvariant();sourceId=sourceId.Trim();if(eventKey.Length>64||sourceType.Length>32||sourceId.Length>190)return;
      DateTime localNow=DateTime.Now,weekDate=ProgressWeekStart(localNow);string week=weekDate.ToString("yyyy-MM-dd",CultureInfo.InvariantCulture),occurred=Now(),key=HashText(eventKey+"|"+sourceType+"|"+sourceId);int cap=eventKey=="person_added"||eventKey=="calendar_item_created"?4:1;
      using(SqliteDb db=Open()){
        db.Exec("BEGIN IMMEDIATE");
        try{
          if(ToInt(db.Scalar("SELECT COUNT(*) FROM local_progress_events WHERE idempotency_key=?",key))>0){db.Exec("COMMIT");return;}
          int used=ToInt(db.Scalar("SELECT COUNT(*) FROM local_progress_events WHERE event_key=? AND week_start=? AND status='awarded'",eventKey,week));
          if(used>=cap){db.Execute("INSERT INTO local_progress_events(event_key,source_type,source_id,idempotency_key,score_value,occurred_at,week_start,status)VALUES(?,?,?,?,0,?,?,'suppressed')",eventKey,sourceType,sourceId,key,occurred,week);db.Exec("COMMIT");return;}
          db.Execute("INSERT INTO local_progress_events(event_key,source_type,source_id,idempotency_key,score_value,occurred_at,week_start,status)VALUES(?,?,?,?,?,?,?,'awarded')",eventKey,sourceType,sourceId,key,score,occurred,week);long eventId=ToLong(db.Scalar("SELECT last_insert_rowid()"));
          Dictionary<string,string> profile=db.QueryOne("SELECT all_time_score,current_streak,longest_streak,last_qualified_week FROM local_progress_profile WHERE id=1");int current=profile==null?0:ToInt(profile["current_streak"]),longest=profile==null?0:ToInt(profile["longest_streak"]),totalAdded=score;string last=profile==null?"":profile["last_qualified_week"];
          db.Execute("INSERT INTO local_progress_weekly_rollups(week_start,score,qualified,updated_at)VALUES(?,?,0,?) ON CONFLICT(week_start) DO UPDATE SET score=score+excluded.score,updated_at=excluded.updated_at",week,score,occurred);
          if(last!=week){string prior=weekDate.AddDays(-7).ToString("yyyy-MM-dd",CultureInfo.InvariantCulture);current=last==prior?current+1:1;longest=Math.Max(longest,current);string weeklyKey=HashText("qualifying_week|"+week);db.Execute("INSERT OR IGNORE INTO local_progress_events(event_key,source_type,source_id,idempotency_key,score_value,occurred_at,week_start,status)VALUES('qualifying_week','local_week',?,?,25,?,?,'awarded')",week,weeklyKey,occurred,week);if(ToInt(db.Scalar("SELECT changes()"))>0){totalAdded+=25;db.Execute("UPDATE local_progress_weekly_rollups SET score=score+25,qualified=1,updated_at=? WHERE week_start=?",occurred,week);}else db.Execute("UPDATE local_progress_weekly_rollups SET qualified=1,updated_at=? WHERE week_start=?",occurred,week);}
          int oldScore=profile==null?0:ToInt(profile["all_time_score"]),newScore=oldScore+totalAdded;db.Execute("UPDATE local_progress_profile SET all_time_score=?,current_streak=?,longest_streak=?,last_qualified_week=?,level_code=?,updated_at=? WHERE id=1",newScore,current,longest,week,ProgressLevel(newScore),occurred);
          int people=ToInt(db.Scalar("SELECT COUNT(*) FROM people WHERE deleted_at IS NULL"));if(people>=1)AwardLocalBadge(db,"first_root",eventId);if(people>=10)AwardLocalBadge(db,"tree_keeper",eventId);if(people>=100)AwardLocalBadge(db,"century_canopy",eventId);if(ToInt(db.Scalar("SELECT COUNT(*) FROM people WHERE deleted_at IS NULL AND length(trim(notes))>=40"))>=5)AwardLocalBadge(db,"memory_keeper",eventId);if(eventKey=="reviewed_plugin_install")AwardLocalBadge(db,"plugin_pathfinder",eventId);foreach(int milestone in new[]{4,12,26,52,104})if(longest>=milestone)AwardLocalBadge(db,"weekly_gardener_"+milestone.ToString(CultureInfo.InvariantCulture),eventId);
          db.Exec("COMMIT");
        }catch{db.Exec("ROLLBACK");throw;}
      }
    }

    internal void SaveProgressVisualMode(string mode){mode=(mode??"").Trim().ToLowerInvariant();if(!new[]{"background","subtle","expressive"}.Contains(mode))throw new InvalidOperationException("Choose a valid Progress display mode.");using(SqliteDb db=Open())db.Execute("UPDATE local_progress_profile SET visual_mode=?,updated_at=? WHERE id=1",mode,Now());ProtectDatabaseFile();}
    internal string ProgressBodyClass(){using(SqliteDb db=Open()){Dictionary<string,string> row=db.QueryOne("SELECT visual_mode,level_code FROM local_progress_profile WHERE id=1 LIMIT 1");if(row==null||row["visual_mode"]=="background")return "";return "progress-"+row["visual_mode"]+" progress-level-"+row["level_code"];}}
    internal Dictionary<string,object> ProgressSummary(){
      Dictionary<string,object> result=new Dictionary<string,object>();using(SqliteDb db=Open()){Dictionary<string,string> profile=db.QueryOne("SELECT visual_mode,all_time_score,current_streak,longest_streak,level_code FROM local_progress_profile WHERE id=1 LIMIT 1")??new Dictionary<string,string>{{"visual_mode","subtle"},{"all_time_score","0"},{"current_streak","0"},{"longest_streak","0"},{"level_code","seed"}};result["visual_mode"]=profile["visual_mode"];result["score"]=ToInt(profile["all_time_score"]);result["current_streak"]=ToInt(profile["current_streak"]);result["longest_streak"]=ToInt(profile["longest_streak"]);result["level"]=CultureInfo.InvariantCulture.TextInfo.ToTitleCase(profile["level_code"]);List<Dictionary<string,string> > weeks=new List<Dictionary<string,string> >();DateTime current=ProgressWeekStart(DateTime.Now);for(int i=11;i>=0;i--){DateTime date=current.AddDays(-7*i);string key=date.ToString("yyyy-MM-dd",CultureInfo.InvariantCulture);Dictionary<string,string> row=db.QueryOne("SELECT qualified FROM local_progress_weekly_rollups WHERE week_start=? LIMIT 1",key);weeks.Add(new Dictionary<string,string>{{"week_start",key},{"label",date.ToString("dd MMM",CultureInfo.CurrentCulture)},{"qualified",row==null?"0":row["qualified"]}});}result["weeks"]=weeks;result["badges"]=db.Query("SELECT badge_key FROM local_progress_badges ORDER BY awarded_at").Select(row=>row["badge_key"]).ToList();}return result;
    }
    ...
    internal void AddPerson(string fullName, string relationship, string birthDate, string place, string notes) {
      ...
      RecordProgressEvent("person_added","person",progressId,20);
      ProtectDatabaseFile();
    }
    ...
    internal void SavePluginInstall(PortablePluginInfo plugin, string entrypoint) {
      ...
      RecordProgressEvent("reviewed_plugin_install","plugin",plugin.slug,25);
      ProtectDatabaseFile();
    }
