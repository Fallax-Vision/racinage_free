...
namespace RacinageFreeDesktop {
  ...
  internal static class PortablePaths {
    ...
    internal static void EnsureMutableFolders() {
      foreach (string path in new[] { Root, DataDir, MediaDir, LogsDir, UpdatesDir, TokensDir, WebViewDir }) {
  ...
  internal sealed class LocalServer {
    ...
    private string FamilyPage(string error) {
      ...
      string body =
        "<section class='dashhead'><div><p class='kicker'>Local family account</p><h1>" + H(family["name"]) + "</h1><p>Saved on this Windows device. Sharing and hosted sync require an upgraded Racinage plan.</p></div><a class='button ghost' href='/logout'>Log out</a></section>" +
        ErrorHtml(error) +
    ...
    private static string Page(string title, string body) {
      return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>" +
        ...
        "<header><a class='brand' href='/'>Racinage Free</a><nav><a href='/'>Home</a><a href='/login'>Login</a><a href='/start-free'>Start Free</a><a href='" + PortablePaths.PricingUrl + "'>Upgrade</a></nav></header>" +
...
*{box-sizing:border-box}body{margin:0;font-family:Segoe UI,Tahoma,sans-serif;background:#f8fbfc;color:var(--text)}a{color:#007584;text-decoration:none}header{height:58px;display:flex;align-items:center;justify-content:space-between;padding:0 28px;border-bottom:1px solid var(--line);background:#fff;position:sticky;top:0;z-index:5}.brand{font-weight:800;color:var(--brand)}nav{display:flex;gap:16px;align-items:center}nav a{font-size:14px;font-weight:600;color:var(--muted)}main{max-width:1120px;margin:0 auto;padding:34px 24px 70px}.hero{min-height:360px;display:grid;align-items:center;border-bottom:1px solid var(--line)}.hero h1,.dashhead h1{font-size:48px;line-height:1.02;margin:6px 0 16px;color:var(--brand);font-family:Georgia,serif}.hero p,.dashhead p,.note{font-size:17px;line-height:1.6;max-width:720px;color:var(--muted)}.kicker{font-size:12px!important;text-transform:uppercase;letter-spacing:.14em;color:var(--accent)!important;font-weight:800;margin:0}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:20px}.button{display:inline-flex;align-items:center;justify-content:center;min-height:40px;padding:0 18px;border-radius:8px;border:1px solid var(--brand);background:var(--brand);color:#fff;font-weight:700;cursor:pointer}.button.ghost{background:transparent;color:var(--brand);border-color:#9ab2b8}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;margin-top:28px}.grid article,.panel{background:#fff;border:1px solid var(--line);border-radius:12px;padding:24px}.grid h2,.panel h2{margin:0 0 10px;color:var(--brand);font-family:Georgia,serif}.grid p,.panel p{line-height:1.55;color:var(--muted)}.narrow{max-width:460px;margin:30px auto}.wide{margin-top:18px}.layout{display:grid;grid-template-columns:1fr 1fr;gap:18px}.dashhead{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:22px}form{display:grid;gap:12px}label{display:grid;gap:6px;font-size:13px;font-weight:700;color:var(--brand)}input,textarea{width:100%;border:1px solid #cad8dd;border-radius:8px;padding:10px 12px;font:inherit;background:#fbfdfd;color:var(--text)}textarea{resize:vertical}.error{border:1px solid #efb5b5;background:#fff2f2;color:#9b2525;border-radius:8px;padding:10px 12px}.sharebar{display:flex;flex-wrap:wrap;gap:8px;margin:12px 0 18px}.share{border:1px solid #cddbe0;background:#f4faff;border-radius:8px;padding:8px 11px;cursor:pointer;font-weight:700;color:var(--brand)}.people{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}.people article{border:1px solid var(--line);border-radius:10px;padding:14px;background:#fbfdfd}.people strong{display:block;color:var(--brand)}.people span{display:block;color:var(--accent);font-size:12px;font-weight:800;text-transform:uppercase;margin-top:3px}.people p{margin:8px 0 0;font-size:14px}.textbtn{border:0;background:transparent;color:#b93333;padding:8px 0 0;cursor:pointer;font-weight:700}.empty{margin:0}.modal{position:fixed;inset:0;background:rgba(5,21,25,.55);display:grid;place-items:center;padding:24px;z-index:20}.modal[hidden]{display:none}.modalbox{width:min(440px,100%);background:#fff;border-radius:12px;padding:24px;border:1px solid var(--line)}.modalbox h2{margin:0 0 10px;color:var(--brand);font-family:Georgia,serif}.panelhead{display:flex;justify-content:space-between;gap:12px;align-items:center}.panelhead h2,.panelhead p{margin:0}@media(max-width:760px){header{padding:0 16px}nav{gap:10px}.hero h1,.dashhead h1{font-size:36px}.layout{grid-template-columns:1fr}.dashhead{display:block}}";
    }
